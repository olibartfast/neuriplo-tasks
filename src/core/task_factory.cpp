...
std::unique_ptr<TaskInterface> TaskFactory::createTaskInstance(
  ...
  // ============ OBJECT DETECTION ============
  // YOLO variants
  if (normalized == "yolo" || normalized == "yolov7e2e" ||
      normalized == "yolov10" || normalized == "yolo26" ||
      normalized == "yolonas" || normalized == "yolov4") {
    return std::make_unique<ObjectDetectionTask>(model_info, normalized);
  }

  // Transformer-based detectors
  if (normalized == "rtdetr" || normalized == "rtdetrul" ||
      normalized == "rfdetr") {
    return std::make_unique<ObjectDetectionTask>(model_info, normalized);
  }
  ...
  if (normalized == "yoloseg" || normalized == "yolov10seg" ||
      normalized == "yolo26seg" ||
