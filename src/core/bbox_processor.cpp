...
cv::Rect BBoxProcessor::calculate_bounding_box(
...
{
    const float scale_width = static_cast<float>(original_size.width) / network_size.width;
    const float scale_height = static_cast<float>(original_size.height) / network_size.height;
    
    return cv::Rect(
        static_cast<int>(bbox.x * scale_width),
        static_cast<int>(bbox.y * scale_height),
        static_cast<int>(bbox.width * scale_width),
        static_cast<int>(bbox.height * scale_height)
    );
}
