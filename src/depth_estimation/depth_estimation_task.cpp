...
std::vector<Result> DepthEstimationTask::decode(const Size& frame_size, const std::vector<Tensor>& tensors) {
...
std::unique_ptr<Preprocessor> DepthEstimationTask::createPreprocessor(ModelType type, const Size& input_size) {
    switch (type) {
...
Size DepthEstimationTask::extractInputSize(const ModelInfo& model_info) {
    int width = 518;
    int height = 518;
