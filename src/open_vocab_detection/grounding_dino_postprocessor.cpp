...
std::vector<OpenVocabDetection> GroundingDinoPostprocessor::postprocess(const std::vector<Tensor>& tensors,
    ...
    const int64_t num_queries = boxes.shape.size() >= 2 ? boxes.shape[boxes.shape.size() - 2] : 0;
    ...
    const size_t boxes_needed = static_cast<size_t>(num_queries * box_dims);
    const size_t logits_needed = static_cast<size_t>(num_queries * seq_len);
