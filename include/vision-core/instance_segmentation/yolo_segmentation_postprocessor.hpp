...
public:
  YoloSegmentationPostprocessor(InstanceSegmentationTask::ModelType model_type,
  ...
  postprocess(const std::vector<Tensor>& tensors,
...
private:
  ...
  std::vector<InstanceSegmentation>
  postprocessYoloSeg(const std::vector<Tensor>& tensors,
  ...
  ///   [1]: Mask Prototypes [1, 32, 160, 160]
  std::vector<InstanceSegmentation>
  postprocessYoloNmsFreeSeg(const std::vector<Tensor>& tensors,
                            const cv::Size& frame_size);
  ...
  cv::Mat generateMask(const std::vector<float>& coeffs,
                       const float* protos_data, 
                       int proto_h,
                       int proto_w);
  ...
  cv::Mat cropAndResizeMask(const cv::Mat& mask,
