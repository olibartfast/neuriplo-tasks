...
class YoloPostprocessor : public Postprocessor {
  public:
    YoloPostprocessor(ObjectDetectionTask::ModelType model_type, const cv::Size& input_size, float confidence_threshold,
                      float nms_threshold);
  ...
  private:
    ...
    std::vector<Detection> postprocessYoloV4(const std::vector<Tensor>& tensors, const cv::Size& frame_size);

    std::vector<Detection> postprocessYoloNmsFree(const Tensor& output, const cv::Size& frame_size);

    std::vector<Detection> postprocessYoloNAS(const Tensor& boxes, const Tensor& scores, const cv::Size& frame_size);
